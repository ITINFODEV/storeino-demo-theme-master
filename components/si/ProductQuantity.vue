<template>
    <div class="flex items-center justify-center">
        <button @click="inc(-1)" class="bg-white-50 border py-2 px-4"
            :class="value > quantity.min ? 'bg-white text-black' : ''">-</button>
        <div class="flex">
            <label class="relative border py-2 h-full">
                <span class="px-2 py-4 px-5 text-center ">{{ value }}</span>
                <input :min="quantity.min" :max="quantity.max"
                    class="px-2 text-center absolute outline-none inset-0 bg-opacity-0 w-full h-full" v-model="value"
                    type="number">
            </label>
            <span class="flex items-center bg-white border px-2" v-if="quantity.unit">{{ quantity.unit }}</span>
        </div>
        <button @click="inc(1)" class="bg-white-50 border py-2 px-4"
            :class="value < quantity.instock ? 'bg-white text-black' : ''">+</button>
    </div>
</template>
