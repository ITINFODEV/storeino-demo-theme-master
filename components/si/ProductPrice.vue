<template>
    <div>
        ...
        <div class="price flex my-2" v-if="type == 'simple'">
            <p>From: </p>
            <b class="text-red-600">{{ price.salePrice }}{{ $store.state.currency.symbol }}</b>
            <span class="flex w-2"></span>
            <span class="relative" v-if="price.comparePrice > 0">

                <b class="text-stone-100">{{ price.comparePrice }}{{ $store.state.currency.symbol }}</b>
